...
const STORAGE_KEY = 'abiding_prayer_ministry_v3';
const SETTINGS_KEY = 'abiding_prayer_settings_v3';
...
export default function JournalingApp() {
  ...
  const tabs = ['home', 'instructions', 'journal', 'progress', 'donate', 'settings'];
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      try {
        ...
        setShowOnboarding(parsed.showOnboarding !== false);
      } catch {}
    }
  ...
  useEffect(() => {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({ notificationsEnabled, showOnboarding }));
  }, [notificationsEnabled, showOnboarding]);
      ...
      <div className="max-w-md mx-auto space-y-6 pb-24">
        {showOnboarding && (
          <div className="bg-stone-900 text-amber-50 rounded-3xl p-5 shadow-2xl border border-stone-700 space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => setShowOnboarding(false)} className="text-amber-200 text-sm">
                Hide
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={openInstall}
                  className="rounded-2xl bg-amber-100 text-stone-900 py-3 font-medium disabled:opacity-60"
                  disabled={!deferredPrompt && !/iPhone|iPad|iPod/i.test(typeof navigator !== 'undefined' ? navigator.userAgent : '')}
                >
                  Install App
                </button>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="rounded-2xl border border-stone-500 text-amber-50 py-3 text-center"
                >
                  Support Missions
                </a>
              </div>
        ...
        {activeTab === 'instructions' && (
          <div className="bg-amber-50 border border-amber-100 rounded-3xl p-5 shadow-sm text-stone-700 space-y-4">
            ...
            <h2 className="text-xl font-serif font-semibold text-stone-800">How to Use Abiding Prayer</h2>
            ...
            <p>Here is a quick reference of the steps for abiding in God to go along with the journaling. Practicing daily will build the habit of living in God’s presence.</p>
            <div>
              ...
              <ul className="list-disc pl-5 space-y-1 text-sm">
                ...
                <li>Guarding our soul according to God’s desires. Not dwelling on negative things, not judging nor obsessing to figure things out. Rather constantly choosing God’s presence and praising Him.</li>
                <li>Talking to God all day long, seeking to stay in conscious contact with Him.</li>
                <li>Asking God for grace for every task.</li>
              </ul>
            </div>
            <div>
              <h3 className="font-semibold mb-2">Each Journaling Page Questions</h3>
              <ul className="list-disc pl-5 text-sm space-y-1">
                <li>Gave thanks?</li>
                <li>Guarded soul?</li>
                <li>Talked to God?</li>
                <li>Asked for Grace?</li>
              </ul>
            </div>
            <div>
            ...
            <p className="italic text-sm">The point is not worrying about how well we do but accepting whatever happens. Then, choosing to be in God’s presence and trusting Him to change you. Acknowledging your weakness so His strength will come.</p>
          </div>
        )}
        ...
        {activeTab === 'journal' && (
          <div className="bg-white/92 backdrop-blur rounded-3xl shadow-2xl p-6 space-y-4 border border-stone-200">
            ...
            <button onClick={addEntry} className="w-full bg-stone-800 text-white rounded-2xl py-3 shadow-md">
              Save Entry
            </button>
        ...
        {activeTab === 'settings' && (
          <div className="bg-white border border-stone-200 rounded-3xl p-6 space-y-4 shadow-xl">
            ...
            <label className="flex justify-between items-center rounded-2xl border border-stone-200 px-4 py-3">
              <span>Show Welcome Card</span>
              <input type="checkbox" checked={showOnboarding} onChange={() => setShowOnboarding(!showOnboarding)} />
            </label>
        ...
        <div className="sticky bottom-3 max-w-md mx-auto bg-white/95 backdrop-blur border border-stone-200 rounded-3xl shadow-lg px-2 py-2 flex justify-around overflow-hidden">
          <div className="absolute top-2 bottom-2 rounded-2xl bg-stone-800 transition-all duration-300" style={{ left: `${tabIndex * (100 / 6)}%`, width: `${100 / 6}%` }} />
          ...
          <TabButton active={activeTab === 'progress'} icon="📈" label="Progress" onClick={() => switchTab(3)} />
          <TabButton active={activeTab === 'donate'} icon="🤍" label="Donate" onClick={() => switchTab(4)} />
          <TabButton active={activeTab === 'settings'} icon="⚙️" label="Settings" onClick={() => switchTab(5)} />
        </div>
      </div>
    </div>
  );
